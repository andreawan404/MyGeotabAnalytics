...
export function initSecurityView(container: HTMLElement, ctx: ViewCtx): () => void {
  ...
  function renderFeed(
    ...
    return `<div class="fa-scroll-table fa-sec-feedwrap">
      ...
      ${
        videoPage
          ? '<p class="fa-note">Tombol <strong>Rekaman</strong> membuka halaman Video MyGeotab, tersaring ke jendela <strong>&plusmn;15 menit</strong> di sekitar insiden. Penyaringan per unit tidak bisa dikirim &mdash; halaman itu memakai id kameranya sendiri, bukan id kendaraan MyGeotab &mdash; jadi cocokkan dengan <strong>nama unit di baris ini</strong> bila ada beberapa klip. Tab kategori sudah dipaksa ke <em>All</em> dan klip yang pernah di-dismiss ikut ditampilkan, supaya tidak ada rekaman yang tersembunyi. Klipnya tidak bisa diputar di sini: MyGeotab tidak mengekspos rekaman kamera ke API publik.'
